...
export const Memory88 = () => {
    const stored_images = JSON.parse(localStorage.getItem('files'))
    const stored_whathappened = localStorage.getItem('whathappened')
  ...
  {stored_images.map((image, index) => (
    <div
      className={`inimage ${selectedImageIndex === index ? 'selected' : ''}`}
      key={index}
      id={`image-${index}`}
      onClick={() => handleImageClick(index)}
    >
      <a href={image.preview}>
        <img src={image.preview} alt='inimage' />
      </a>
    </div>
  ))}
          </div>
